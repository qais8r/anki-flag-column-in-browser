...
def _row_base_color(card_id: int) -> dict[str, str] | None:
    if aqt.mw is None or aqt.mw.col is None:
        return None
    row = aqt.mw.col.db.first(_BROWSER_ROW_STATE_SQL, card_id)
    if row is None:
        return None
    queue, is_marked = row
    if is_marked:
...
class FlagIconDelegate(StatusDelegate):
    ...
    def _glyph_path(
        ...
        if cached is not None:
            return cached
        path = QPainterPath()
        path.addText(0, 0, font, _FLAG_GLYPH)
...
def _on_browser_did_fetch_row(card_or_note_id, is_note, row, _columns) -> None:
    row._flag_indicator = 0
    if is_note:
        return
    flag_index = _FLAG_INDEX_BY_BG_KEY.get(_color_key(row.color))
    if flag_index is None:
        return
    row._flag_indicator = flag_index
    row.color = _row_base_color(card_or_note_id)
